...
window_number="#($SCRIPTS_PATH/custom-number.sh #I -r)"
custom_pane="#($SCRIPTS_PATH/custom-number.sh #P -o)"
zoom_number="#($SCRIPTS_PATH/custom-number.sh #P -O)"

#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#[fg=#1F2335,bg=#f7768e,bold] #{?client_prefix,󰠠 ,#[dim]󰤂 }#[fg=#1F2335,bg=#f7768e,bold,nodim]#S $RESET"

#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "#[fg=#7aa2f7,bg=#1F2335]   #[fg=#a9b1d6]$window_number #[bold,nodim]#W#[nobold,dim]#{?window_zoomed_flag, $zoom_number, $custom_pane} #{?window_last_flag,,} "
# Unfocused
